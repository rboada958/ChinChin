...
abstract class BaseRemoteRepository {
        ...
        return try{
            ...
            withContext(Dispatchers.Main){
                ...
                when(e){
                    is HttpException -> {
                        when {
                            e.code() == 401 || e.code() == 400 -> {val body = e.response()?.errorBody()
    ...
    fun getErrorMessage(responseBody: ResponseBody?): String {
        return try {
            ...
            when {
                jsonObject.has(FIELDS_ERRORS) -> jsonObject.getJSONArray(FIELDS_ERRORS).getJSONObject(0).getJSONArray(
                    CONSTRAINS).getString(0)
